...
MNML_USER_CHAR="${MNML_USER_CHAR:-∫Σλ}"
...
alias som="nvim ~/.zshrc"
alias conf="nvim ~/.config/nvim/"
alias conft="nvim ~/.tmux.conf"
alias prog="nvim ~/Desktop/Nivesh\ Birangal/prog.txt"
...
# weather
# alias weather="curl -s wttr.in/"$(curl -s ipinfo.io/city)"|grep -v 'Follow\|Location:'"
# weather_function(){
# curl -s wttr.in/$@ | grep -v 'Follow\|Location'
# }

function weather_display() {
    local input="$1"  # Take the first argument as input

    if [ -z "$input" ]; then
        # Optionally, you can handle the case where input is empty here
        curl -s wttr.in/"$(curl -s ipinfo.io/city)"|grep -v 'Follow\|Location:'
        return 0
    else
        curl -s wttr.in/$input|grep -v 'Follow'
        return 0
    fi
}
alias weather="weather_display"
alias skim='/Applications/Skim.app/Contents/MacOS/Skim'
# echo "hello $USER"
# echo "hello $USER - do it or dont the time will pass anyway"
...
# Created by `pipx` on 2024-10-05 05:16:31
# export PATH="$PATH:/Users/niveshbirangal/.local/bin"
